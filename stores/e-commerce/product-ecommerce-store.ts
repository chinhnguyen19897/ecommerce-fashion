...
export const useProductEcommerceStore = defineStore(
  "product-ecom-store",
  () => {
    ...
    async function fetchProducts(
      ...
      const { data, refresh } = await useFetch(
        "/api/e-commerce/product/get-products",
        {
          query: {
            ...params,
            page: page.value,
    ...
    async function fetchProductWithSameCategory(categoryId: number) {
      const { data, refresh } = await useFetch(
        "/api/e-commerce/product/get-same-category-product",
        {
          query: {
    ...
    async function fetchSingleProductData(slug: string) {
      const { data, refresh } = await useFetch(
        "/api/e-commerce/product/single-product",
        {
          query: {
            slug: slug,
          },
