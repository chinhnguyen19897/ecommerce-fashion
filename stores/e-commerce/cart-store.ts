...
export const useCartStore = defineStore("cart-store", () => {
  ...
  function addToCart(product: Record<string, any>) {
    ...
    if (productExist.length === 0) {
      const updateTotalPrice =
        parseFloat(product?.price) * defaultQuantity.value;
      cartData.value.push({
        ...product,
  ...
  function addQuantity(productId: number, newQuantity: number) {
    ...
    if (newArr.length > 0) {
      ...
      const updateTotalPrice =
        parseFloat(totalProductPrice) + parseFloat(totalProductPrice);
    ...
    if (newArr.length > 0) {
      if (newQuantity <= parseFloat(newArr[0].quantity)) {
        ...
        if (updateQuantity === 0) {
          ...
          const updateTotalPrice =
            parseFloat(totalProductPrice) - parseFloat(restProps.price);
          const updateProduct = cartData.value.filter(
            (i) => i.id !== productId,
          );
          cartData.value = [];
          cartData.value = [...updateProduct];
  ...
  function getTotalPrice() {
    ...
    cartData.value.forEach((product) => {
      totalPrice.value += product?.totalAmount;
    });
  }
